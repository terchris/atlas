...
- **Migration numbering in a multi-agent repo**: Phase 2.1 says "check next free number" rather than hard-coding 025. Per the [Multi-agent repo memory](..../.claude/projects/-Users-terje-christensen-learn-helpers-atlas/memory/feedback_multi-agent-repo.md), another branch may have taken 025 by the time this PLAN runs. Ping on talk.md if a collision risk appears.
...
- **NGO-specific vs generic-parameterised**: investigation [Q4] explicitly resolves this — NF-specific for v1. `brreg-ngo-units` parameterised by NGO slug is a follow-up when the third NGO needs it (likely N.K.S. per [`ngo-landscape.md`](../../../../docs/research/ngo-landscape.md)).
