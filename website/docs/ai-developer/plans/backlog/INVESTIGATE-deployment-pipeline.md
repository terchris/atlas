...
5. **v1 deploy surface narrowed (2026-05-11)** — both Next.js frontends are **kept in the repo but not deployed in v1**. The customer surface for humans is the Docusaurus site (hostname pivoted to `atlas.sovereignsky.no` in item 9 below).
...
9. **Docusaurus deploy moves to GitHub Pages at `atlas.sovereignsky.no`** (2026-05-12) — UIS itself ships its docs at `uis.sovereignsky.no` via GitHub Pages; Atlas follows the same sister-project pattern. The Docusaurus row in the artefacts table is no longer UIS-Traefik-served; it's `actions/deploy-pages@v4` with a custom-domain CNAME. The dynamic services (PostgREST, Dagster, atlas-data) remain on UIS — only the static docs pivot. The earlier `atlas.helpers.no`-via-UIS-Traefik plan is deferred; could revisit if there's ever a reason to migrate docs to helpers.no infrastructure.
...
21. **PostgREST CORS / spec exposure** — for the Scalar page at `atlas.sovereignsky.no/api` to call PostgREST on `api-atlas.helpers.no` from the browser, PostgREST needs CORS configured to allow the `atlas.sovereignsky.no` origin (and `http://localhost:3000` for local Docusaurus dev). Coordinate with UIS — likely a one-line config change in their Helm values.
    ...
    **What UIS needed to change**: configure PostgREST to send `Access-Control-Allow-Origin` on all responses, not just OPTIONS.

    ✅ **RESOLVED 2026-09-23.** Measured against the live public API with a browser `Origin` header — GET responses carry `access-control-allow-origin: *`, on the spec endpoint and on data endpoints, JSON and CSV alike. UIS `1.6.143` separately set `openapi-server-proxy-uri`, so the live document now names the public host instead of a bind-all placeholder.

    🔵 **Both fixes together removed the workaround rather than improving it.** Scalar now reads the live spec, and `website/static/openapi.json` plus `scripts/snapshot-openapi.mjs` were **deleted** (atlas#431). Confirmed by headless render before deletion: 19 of 19 relations painted, and the `:443` in the live document's `host` — which exists nowhere else — present in the DOM, proving the fetch went to the API rather than to the committed copy.

    ⚠️ **Why this mattered beyond one page.** The snapshot was a generated copy of live state committed to the repo, and it drifted three times in a single evening: on a deploy, on a config default that outlived its reason, and on a transform (urb-agents #1407, #1409, #1414). A liveness gate was designed to police it and then not built — *a copy you no longer keep cannot go stale.*
...
1. **Docusaurus skeleton + deploy** — landing page + nav stub, live at `atlas.sovereignsky.no` via GitHub Pages. Gets the deploy plumbing proven (GHA build → `actions/deploy-pages@v4` → custom-domain CNAME) on the lowest-stakes content. ~1–2 days.
...
- [`website/docs/ai-developer/plans/backlog/INVESTIGATE-developer-docs-surface.md`](INVESTIGATE-developer-docs-surface.md) — narrative-docs content scope. Now served from Docusaurus root on `atlas.sovereignsky.no` (via GitHub Pages); the file needs reconciling (it still names a separate `developer-atlas.helpers.no` hostname that this plan no longer uses).
