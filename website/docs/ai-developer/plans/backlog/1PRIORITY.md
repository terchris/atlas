...
| 🔴 **All Brreg organisations in Atlas?** — scope, personal data, and what is published | [INVESTIGATE-all-brreg-organisations](INVESTIGATE-all-brreg-organisations.md) | 2026-09-10 | Terje asked how Atlas could hold every organisation in Brreg and pointed at his own `terchris/shadow-brreg` as the reference. Architecture analysed and sound — bulk snapshot + `oppdateringer` change feed with a watermark. **Three decisions are his and none are technical**: ingest scope (all ~1.1M / Frivillighetsregisteret only / keep the curated 11) and what if anything reaches `api_v1` — ingest is reversible, publication is not. ✅ **The personal-data concern I raised was overstated and Terje corrected it**: the persons are in the register because the law requires it and it is public NLOD data. What survives is an attribution gap — `seed-sources/brreg-enheter/` has no manifest and declares no licence, unlike every `sources/*` module. Live today at 122 rows. |
