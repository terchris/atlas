...
| 🔴 **All Brreg organisations in Atlas?** — scope, personal data, and what is published | [INVESTIGATE-all-brreg-organisations](INVESTIGATE-all-brreg-organisations.md) | 2026-09-10 | Terje asked how Atlas could hold every organisation in Brreg and pointed at his own `terchris/shadow-brreg` as the reference. Architecture analysed and sound — bulk snapshot + `oppdateringer` change feed with a watermark. **Three decisions are his and none are technical**: ingest scope (all ~1.1M / Frivillighetsregisteret only / keep the curated 11); ⚠️ **whether Atlas holds personal data at scale** — Enhetsregisteret's ENK entries are natural persons; and what if anything reaches `api_v1`. Ingest is reversible, publication is not. |
