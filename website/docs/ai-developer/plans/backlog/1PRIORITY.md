...
| 🔴 **All Brreg organisations in Atlas** — scope, and what is published | [INVESTIGATE-all-brreg-organisations](INVESTIGATE-all-brreg-organisations.md) | 2026-09-10 | Terje: *"focus now is taking the concept that i wrote in that repo many years ago and implementing it in atlas."* Architecture analysed and **re-verified against the live Brreg API 2026-09-11** — unchanged in shape, bulk file 210 MB gzipped regenerated daily. ✅ **Both blocking questions answered**: the change feed has **no retention window** (goes back to 2018), so cadence is a freshness choice not a correctness one; and it accepts `?oppdateringsid=`, which is a better watermark than the date shadow-brreg used and yields backlog depth for free. **Two decisions remain his**: ingest scope, and what reaches `api_v1`. |
