 ...
 *
 * Prose in website/data/*.yaml carries curl examples, and those examples used
 * to hardcode the API hostname — which is how `api-atlas.sovereignsky.no`
 * survived in the published catalogue even after the generator's own constant
 * was fixed. A contributor writing a new example has nowhere else to put a URL,
 * so give them a token rather than a literal to copy.
 *
 * Write `{{API_BASE}}` / `{{SITE_BASE}}` in the YAML; they expand here.
 */
function expandHostTokens(text) {
  return text
    .replaceAll('{{API_BASE}}', ATLAS_API_BASE_URL)
    .replaceAll('{{SITE_BASE}}', ATLAS_SITE_BASE_URL);
}

// Hostnames live in website/hosts.mjs — see there for why they are not here.
const ATLAS_BASE_URL = ATLAS_SITE_BASE_URL;
const POSTGREST_BASE_URL = ATLAS_API_BASE_URL;
...
function loadCollections(sources, views) {
  ...
  return list.map((entry) => {
    ...
    return {
      ...
      datasets,
      join_recipe: expandHostTokens((entry.join_recipe ?? '').trim()),
    };
  });
}
