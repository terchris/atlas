...
COMMENT ON COLUMN api_v1.brreg_enhet.paategninger IS 'Official remarks Brreg has attached to the organisation, as jsonb objects carrying `tekst`. 🔴 jsonb rather than text[] for the same reason as `historiske_navn`, and getting this wrong would have flattened enforcement notices into unparseable strings. ⚠️ Present on 100% of rows and NON-EMPTY on about 2 600 — around 0.2%. The column is `[]` for almost every organisation, so "present" and "has a remark" are different questions and only the second is usually the one being asked. The non-empty count moves as the delta job rewrites rows: three observations on 2026-09-24 gave 2 599, 2 600 and 2 598.';
