...
-- indicators__bufdir_barnefattigdom — the per-source indicator relation for `bufdir-barnefattigdom`, published
...
--
-- 🔴 stability:source. THE COLUMN SET HERE FOLLOWS BUFDIR-BARNEFATTIGDOM, NOT ATLAS.
...
-- source_id=bufdir-barnefattigdom, and through the published ref_* lists where one exists.
--
-- 🔵 A VIEW, not a copy — the indicator model is already a table in marts.

select *
from {{ ref('indicators__bufdir_barnefattigdom') }}
