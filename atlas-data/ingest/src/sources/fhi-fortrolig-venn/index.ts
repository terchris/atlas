/**
 * FHI Folkehelsestatistikk table 354 — FORTROLIGVENN_Ungdata_KH. Share of
 * Ungdata respondents reporting they have a "fortrolig venn" — a close
 * friend to confide in about personal matters. Key social-connectedness
 * indicator and a known protective factor against mental-health risk.
 * Same Ungdata cohort as fhi-livskvalitet / fhi-depresjon / fhi-smertestillende.
 */
...
export const SOURCE_ID = "fhi-fortrolig-venn";
const FHI_SOURCE_ID = "nokkel";
const FHI_TABLE_ID = 354;
const TARGET_TABLE = "raw.fhi_fortrolig_venn";
const OUTPUT_PATH = resolve(
  dirname(fileURLToPath(import.meta.url)),
  "../../../output/fhi-fortrolig-venn.ndjson",
);
const WRITE_COLUMNS = [
...
export async function run() {
  return recordIngestRun(SOURCE_ID, async () => {
    ...
    // Cell budget: 409 GEO × 14 AAR × 3 KJONN × 1 ALDER × 1 HARVENN ×
    ...
    const resp = await fetchFhiTableData({
      ...
      request: {
        dimensions: [
          ...
          { code: "HARVENN", filter: "all", values: ["*"] },
...
function toRow(px: PxRow): Row {
  ...
  const harvenn = px.dimensions["HARVENN"];
  const soes = px.dimensions["SOES"];
  const measure = px.dimensions["MEASURE_TYPE"];
  if (!geo || !aar || !kjonn || !alder || !harvenn || !soes || !measure) {
