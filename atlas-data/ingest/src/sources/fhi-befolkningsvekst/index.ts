/**
 * FHI Folkehelsestatistikk table 185 — Befolkningsvekst (population growth).
 * Year-over-year change in resident population by region. Both absolute count
 * (TELLER) and percent rate (RATE) are pulled — small enough to take full
...
export const SOURCE_ID = "fhi-befolkningsvekst";
const FHI_SOURCE_ID = "nokkel";
const FHI_TABLE_ID = 185;
const TARGET_TABLE = "raw.fhi_befolkningsvekst";
const OUTPUT_PATH = resolve(
  dirname(fileURLToPath(import.meta.url)),
  "../../../output/fhi-befolkningsvekst.ndjson",
);
const WRITE_COLUMNS = [
...
export async function run() {
  return recordIngestRun(SOURCE_ID, async () => {
    ...
    // Cell budget: 409 GEO × 23 AAR × 1 KJONN × 1 ALDER × 2 MEASURE
    // ≈ 18.8k. Comfortably under FHI's 50k cap; full history pulled
    // because population growth is a trend metric — single-year snapshots
    // lose the analytical value.
