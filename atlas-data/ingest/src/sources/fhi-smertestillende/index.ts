/**
 * FHI Folkehelsestatistikk table 390 — Smertestillende_ungdata. Share of
 * Ungdata respondents reporting they use painkillers at least weekly —
 * a known marker of chronic pain / psychological distress in adolescents.
 * Same Ungdata cohort base as fhi-livskvalitet, fhi-depresjon, fhi-mobbing.
 */
...
export const SOURCE_ID = "fhi-smertestillende";
const FHI_SOURCE_ID = "nokkel";
const FHI_TABLE_ID = 390;
const TARGET_TABLE = "raw.fhi_smertestillende";
const OUTPUT_PATH = resolve(
  dirname(fileURLToPath(import.meta.url)),
  "../../../output/fhi-smertestillende.ndjson",
);
const WRITE_COLUMNS = [
...
export async function run() {
  return recordIngestRun(SOURCE_ID, async () => {
    ...
    // Cell budget: 409 GEO × 12 AAR × 3 KJONN × 1 ALDER × 1 STATUS ×
    // 1 SOES × 2 MEASURE = 29,448 cells. Trivial — full series pulled.
...
function toRow(px: PxRow): Row {
  ...
  const statusDim = px.dimensions["STATUS"];
  const soes = px.dimensions["SOES"];
  const measure = px.dimensions["MEASURE_TYPE"];
  if (!geo || !aar || !kjonn || !alder || !statusDim || !soes || !measure) {
