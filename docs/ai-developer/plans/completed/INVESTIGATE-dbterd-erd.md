...
**Completed**: 2026-04-23 — adopted dbterd; Mermaid ERD lives at [`docs/stack/erd.md`](../../../stack/erd.md), regenerated via [`atlas-data-repo/dbt/regenerate-erd.sh`](../../../../atlas-data-repo/dbt/regenerate-erd.sh). See [PLAN-dbterd-install-and-document.md](../completed/PLAN-dbterd-install-and-document.md) for the implementation details.

**Origin**: While reviewing the lineage graph in `dbt docs serve` after PLAN-003, the user observed that lineage shows transformation flow but not entity relationships with cardinality. dbt docs ships no ERD view. The `relationships:` tests added in PLAN-003 ([`atlas-data-repo/dbt/models/indicators/schema.yml`](../../../../atlas-data-repo/dbt/models/indicators/schema.yml)) and [`atlas-data-repo/dbt/seeds/schema.yml`](../../../../atlas-data-repo/dbt/seeds/schema.yml) make Atlas's marts schema unusually well-suited to auto-ERD: dbterd's default algorithm reads exactly those tests.
...
**Install.** `uv pip install dbterd` into the existing `atlas-data-repo/dbt/.venv/`. No system packages, no daemons.

**Config file.** Supports `.dbterd.yml` or a `[tool.dbterd]` section in `pyproject.toml` so the CLI flags don't have to be repeated. Atlas would put one in `atlas-data-repo/dbt/`.
...
- Run `dbterd run -t mermaid -ad target -o ../../docs/stack -ofn erd.md -s schema:marts` from `atlas-data-repo/dbt/`.
- Add an npm-style script to `atlas-data-repo/dbt/README.md` (and as a target if we ever add a Makefile).
...
2. **Create `atlas-data-repo/dbt/.dbterd.yml`** with the canonical config: `target: mermaid`, `select: schema:marts`, `resource_type: [model, source]` (extend with `seed` after empirical verification — see step 4), `output: ../../docs/stack`, `output_file_name: erd.md`.
...
5. **Document the workflow in [`atlas-data-repo/dbt/README.md`](../../../../atlas-data-repo/dbt/README.md)**: one paragraph + the regeneration command. Cross-link from [`docs/stack/naming-conventions.md`](../../../stack/naming-conventions.md) so contributors looking at the vocabulary see "the ERD shows current state".
...
- [ ] **PLAN-dbterd-install-and-document.md** — Install dbterd into the dbt venv (pin in `requirements.txt`). Add `.dbterd.yml`. Run once, eyeball the output, commit `docs/stack/erd.md`. Test seed support empirically; finalise the config based on what works. Add a "Regenerating the ERD" section to `atlas-data-repo/dbt/README.md`. Consider adding a `relationships` test for `fact_kommune_indicators.kommune_nr → dim_kommune.kommune_nr` so the fact's edges show up too.
...
- `atlas-data-repo/dbt/models/marts/schema.yml` — *maybe* add `relationships` test for `fact_kommune_indicators.kommune_nr` (open question 2)

---

## Cross-references

- [`atlas-data-repo/dbt/models/indicators/schema.yml`](../../../../atlas-data-repo/dbt/models/indicators/schema.yml) — the relationship metadata dbterd's default algorithm reads.
- [`atlas-data-repo/dbt/seeds/schema.yml`](../../../../atlas-data-repo/dbt/seeds/schema.yml) — seed-side metadata; relevant if `--resource-type seed` works.
- [`docs/stack/naming-conventions.md`](../../../stack/naming-conventions.md) — the canonical vocabulary the ERD should reflect.
